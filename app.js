...
function assertRunnableApp() {
  if (!isRunnableProtocol()) {
    throw new Error('Please run the app with `php -S localhost:8000` instead of opening index.html directly.');
  }
}
...
async function parseJsonResponse(response) {
  ...
  try {
    ...
    throw new Error('The check-in service returned an unexpected response. Make sure PHP is serving the app.');
  }
}
